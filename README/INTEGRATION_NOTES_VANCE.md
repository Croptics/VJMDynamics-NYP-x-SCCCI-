...
> **Note from JQ (2026-07-28, at v2 integration time):** this is Vance's own doc,
> copied verbatim from his v2 branch. Three things read differently in main:
> (1) his page files live under `frontend/src/pages/desktop/` here, not
> `frontend/src/pages/`; (2) the ChatBubble Layout mount he asks for already
> exists (main kept its own draggable ChatBubble shell, hosting his
> AssistantConversation + unread badge + call overlay), and MusterChat is also
> a sidebar nav item (`/assistant`, gated `viewChatbot`); (3) main's
> `getDashboard` is already 5-status-aware, but his snapshot's own KPI
> partition was kept anyway since it also feeds his coach-scoped answers.
> See `INTEGRATION_NOTES.md`'s "Vance v2 integration" section for what was
> deliberately NOT taken from his branch.
