...
import { requireAuth, makeToken, accountFromReq } from "../lib/auth.js";
import { all as dbAll, get as dbGet, run as dbRun } from "../db/connection.js";
import { getAccountByUsername } from "../db/accounts.js";
...
router.post("/api/passkeys/login/verify", wrap(async (req, res) => {
  ...
  if (acc.status && acc.status !== "ACTIVE" && acc.status !== "APPROVED") {
    return fail(res, 403, "ACCOUNT_" + acc.status, "This account isn't active.");
  }
