...
/* ---- Auth ---------------------------------------------------------------- *
 * Mirrors the token format issued by JQ's POST /api/auth/login
 * (`demo.<base64(username)>.token`) by reusing her exported account helpers.
 * No new login flow, no duplicated password logic.
 * -------------------------------------------------------------------------- */
async function accountFromToken(token) {
  ...
  let username;
  try { username = Buffer.from(parts[1], "base64").toString("utf8"); } catch { return null; }
...
async function requireAuth(req, res, next) {
  ...
  // EventSource cannot set headers, so the SSE route passes ?token=
  const token = m ? m[1] : req.query.token;
  const acc = await accountFromToken(token);
...
router.get("/api/exceptions/stream", wrap(requireAuth), (req, res) => {
  res.set({
...
/** GET /api/trips/:id/exceptions — list tickets (+ counts). */
router.get("/api/trips/:id/exceptions", wrap(requireAuth), wrap(async (req, res) => {
 ...
 */
router.get("/api/trips/:id/exceptions/critical-count", wrap(requireAuth), wrap(async (req, res) => {
...
router.get("/api/exceptions/:id", wrap(requireAuth), wrap(async (req, res) => {
...
/** POST /api/trips/:id/exceptions — raise a ticket; CRITICAL pushes to devices. */
router.post("/api/trips/:id/exceptions", wrap(requireAuth), wrap(async (req, res) => {
...
/** PATCH /api/exceptions/:id — resolve, or update priority/note. */
router.patch("/api/exceptions/:id", wrap(requireAuth), wrap(async (req, res) => {
...
router.delete("/api/exceptions/:id", wrap(requireAuth), wrap(async (req, res) => {
 ...
 * -------------------------------------------------------------------------- */
router.post("/api/checkins/manual", wrap(requireAuth), wrap(async (req, res) => {
