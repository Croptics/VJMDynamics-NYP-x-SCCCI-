...
export default function QRCheckInPage() {
  ...
  const ScannerView = (
    <div style={S.scanner}>
      ...
      <div style={S.viewport}>
        ...
        {scanMode === "qr" && (
          /* Jayden's QR module — live scanner. Self-contained: it manages its
             own camera and only mounts in QR mode, so Vimal's face pipeline is
             never affected. */
          <QRScannerPanel
            tripId={TRIP_ID}
            coachId={activeCoachId}
            coachLabel={coach?.coachLabel}
            onCheckedIn={() => { fetchOverview(); fetchCoach(activeCoachId); }}
          />
        )}

        {scanMode === "manual" && (
          /* Jayden's manual attendance tracking — self-contained; reads the
             roster from the already-loaded coach prop, no face code touched. */
          <ManualTrackingPanel
            tripId={TRIP_ID}
            coach={coach}
            coachLabel={coach?.coachLabel}
  ...
  const IssuesView = (
    /* Jayden's Issues tab — mobile exception logging (Figma Screen 10). */
    <IssuesPanel
      tripId={TRIP_ID}
      coachId={activeCoachId}
      coach={coach}
      onLogged={() => { fetchOverview(); fetchCoach(activeCoachId); }}
    />
  );
